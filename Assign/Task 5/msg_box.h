...
typedef struct s_msg_box {
  ...
  char *buf;			/* Buffer allocated by msb_box_init to store
				 * *1* message */
  ...
  pthread_cond_t not_empty;	/* Awake a task blocked on the message box en
				 * a message arrives */
...
int msg_box_receive (msg_box mbox, char *buf, bool block);
