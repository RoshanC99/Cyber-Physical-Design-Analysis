...
class RobotariumABC(ABC):
    ...
    def __init__(self, number_of_robots=-1, show_figure=True, sim_in_real_time=True, initial_conditions=np.array([])):
        ...
        if (self.show_figure):
            ...
            pkw_w = 0.2
            self.axes.add_patch(
                patches.Rectangle([rw_x, rw_y], rw_w, rw_h, edgecolor='None', facecolor=[.86, .86, .86]))
            self.axes.add_patch(
                patches.Rectangle([rw_x + rw_w, rw_y], pkw_w, rw_h, edgecolor='None', facecolor=[.7, 1, .7]))

            # no fly zone
            self.axes.add_patch(patches.Rectangle([rw_x, rw_y + rw_h], rw_w + pkw_w, rw_h / 2, edgecolor='None',
                                                  facecolor=[1, .35, .35]))
            self.axes.add_patch(patches.Rectangle([rw_x, rw_y - rw_h / 2], rw_w + pkw_w, rw_h / 2, edgecolor='None',
                                                  facecolor=[1, .35, .35]))
            self.axes.add_patch(
                patches.Rectangle([rw_x + rw_w + pkw_w, rw_y - rw_h / 2], rw_h / 2, rw_h * 2, edgecolor='None',
                                  facecolor=[1, .35, .35]))
                ...
                p = patches.Rectangle((self.poses[:2, i] + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i] + math.pi / 2), np.sin(self.poses[2, i] + math.pi / 2))) + \
                            ...
                            (-np.sin(self.poses[2, i] + math.pi / 2), np.cos(self.poses[2, i] + math.pi / 2)))),
                                      ...
                                      (self.poses[2, i] + math.pi / 4) * 180 / math.pi, facecolor='#FFD700',
                                      edgecolor='k')
            ...
            self.boundary_patch = self.axes.add_patch(
                patches.Rectangle(self.boundaries[:2], self.boundaries[2], self.boundaries[3], fill=False, linewidth=3))
